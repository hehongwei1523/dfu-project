...
void Receiver(BCSPStack * stack)
{
	...
	{
		//printf("Rcv - dumping while choked pkt %p seq %d ack %d chan %d\n",pkt,getSeq(pkt),getAck(pkt),getProtocolId(pkt)) ;
	...
	else
	{
		...
		if (isUnreliable(pkt))
		{
			//printf("Rcv - got unreliable pkt %p seq %d ack %d chan %d\n",pkt,getSeq(pkt),getAck(pkt),getProtocolId(pkt)) ;
		...
		else
		{
			...
			if (getSeq(pkt) == stack->txack) 
			{
				...
				//printf("Rcv - got reliable pkt %p seq %d ack %d chan %d time %d\n",pkt,getSeq(pkt),getAck(pkt),getProtocolId(pkt)) ;
			...
			else
			{
				...
				//printf("Rcv - expecting %d : dumping reliable pkt %p seq %d ack %d chan %d time %d\n",stack->txack,pkt,getSeq(pkt),getAck(pkt),getProtocolId(pkt)) ;
